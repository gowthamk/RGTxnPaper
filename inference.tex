...
$\C{a.bal + pc*a.bal}\ge 0$, and verify that \C{add\_interest}
...
operation (as described above) is generated. The constraint relates
the predicates $\G_1$ and $\G_2$, representing $s_1$ and $s_2$
(resp.), to the uninterpreted predicate $g$ that represents $s$. The
constraint is assigned a name ($\pi$) so that it can be enabled only
for the choice of free variables ($\nubar$) for which $g$ needs to be
defined; observe that the lambda returned by
$\mssemof{\nubar}{s_1 \bind \lambda x.s_2}$ asserts $\pi(\vbar)$ only
in conjunction with $g(\vbar,r)$, for some $r$.

The first-order encoding of the $\existsl$ expression eliminates the
existential by Skolemizing it. Skolemizing is the process of
substituting an existentially bound $x$ in $\phi\in\Prop^1$ with
$f(\nubar)$, where $f$ is a fresh uninterpreted function, and $\nubar$
are the free variables in $\phi$ bound by enclosing universal
quantifiers. Due to the decidability restrictions that will become
clear shortly, the only uninterpreted functions we admit in our logic
are boolean (i.e., predicates/relations). Consequently, Skolemizing an
existentially bound $x:T$ with $f(\nubar):T$, where $T$ is a
non-boolean type, is not really an option. Instead, we define $f$ as
an uninterpreted relation that behaves as a partial function:
\begin{smathpar}
  \begin{array}{c}
  ...
  \end{array}
\end{smathpar}
We then replace the existentially bound $x$ with a new universally
bound $a$ in $\phi$, under the condition that $f(\nubar,a)$ holds.
Due to the partial function property of $f$, there can be utmost one
instantiation for $a$, as desired. There can however be no
instantiations, i.e., no $a$ that satisfies $f(\nubar,a)$, thus
violating the exists property. Fortunately, this problem can be solved
in our case by observing that the existential always occurs in the
context (antecedent) in the verification conditions (implications)
generated by the \rulelabel{RG-Txn} rule. Intuitively, we prove that a
set of records is equal to, or contained in, other set of records
(e.g., $\F_1 = \F_2$ or $\F \subseteq G$) \emph{assuming} that there
exists a state $\stg$ that satisfies $I$. If there does not exist such
a state, then our premise itself is false, and the goal must be
vacuously provable. We make use of this observation, and let the
context derive $\bot$ when there does not exist a desired $a$. 
