  ...
  \begin{array}{lcl}
    ...
    & & \hspace*{1.45in}}
              {s' = \langle \C{s\_i\_id}=s.\C{s\_i\_id};\, 
                            \C{s\_d\_id}=s.\C{s\_d\_id};\,
                            \C{s\_qty} = \C{k1}\rangle\\
    & & \hspace*{1.45in}} {\bot}) \Rightarrow g_0(\C{item\_req},s')\\
...
\begin{smathpar}
  \begin{array}{lcl}
    ...
    & & \hspace*{0.2in}\forall \C{ireq}. \forall s.~ 
        \pi_3 \Leftrightarrow (\C{ireq} \in \C{item\_reqs} \conj 
        (\G_U(\C{ireq},s') \vee \G_I(\C{ireq},s'))
        \Rightarrow g_1(s))\\
    & & \conj \hspace*{0.05in}\forall s.\exists \C{ireq}. ~ 
        \pi_4 \Leftrightarrow (g_1(s) \Rightarrow (\C{ireq} \in \C{item\_reqs} 
        \conj (\G_U(\C{ireq},s') \vee \G_I(\C{ireq},s')))\\
