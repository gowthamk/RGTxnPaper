...
quantification that binds a set $\stg$ satisfying proposition $\phi$ in a
...
states, respectively. Constant sets can be written using set
literal expressions. For example, the set $\{1,2\}$ can be written as
...
semantics-preserving translation amenable for automated verification.
...
rely relation $\R$, and the high-level invariant $I$ for reasons
...
the same set of records before and after an interference. The
equivalence in $\SL$ translates to equality in first-order logic, as
we describe below. In the inference rules, stability is enforced
constructively by a meta-function $\stabilize{\cdot}$, which accepts
an effect and returns a new effect that is guaranteed to be stable
under $\R$.  $\stabilize{\cdot}$ achieves the stability guarantee by
abstracting away the bound global state ($\stg$) in an unstable $\F$
to an existentially bound $\stg'$ as described below:
\begin{smathpar}
\begin{array}{lcll}
...
the existentially bound $\stg'$. The example in
...
$c_1$ and $c_2$, respectively, except that $\F_2$ is applied to a
...
iteration (recall that we make a local context independence assumption
about database programs; Sec. ~\ref{sec:opsem}), sequential
composition of the effects of different iterations is the same as their
parallel composition. Since the loop body is executed once per each
$z\in x$, the effect of the the loop is a union of effects ($\F$)
for all $z\in x$, all applied to the same state ($\stl$ and $\stg$).
That is, $\F_{loop}(\stl,\stg) = \bigcup_{z\in
...
\RULE
{
   ...
   c \Longrightarrow_{\langle i,\R_e,I \rangle}\F \spc \stable(\R_c,\F)\\
...
$\exists$ quantifier is utmost two more than the number of free
...
is extended only in the encoding rules for $\bind$ and $\existsl$
expressions. Since $\existsl$ is a result of a particular choice of the
weakening semantics (Sec.~\ref{sec:inference}), let us ignore it for now to
focus on the core set expression language. This leaves $\bind$ as the
only case where $\nubar$ is extended.  Since an occurrence of $\bind$
adds one to $|\nubar|$, if $s$ is a subexpression of a top-level
{\sf bind} expression (like $\F_U(\stl,\stg)$ from the above example),
then the prenex signature of $\phi_s$ is $\forall^2\exists$.
Furthermore, if the subexpressions of $s$ are not {\sf bind} expressions,
then none of the $\phi_i$'s are quantified, and the prenex signature
of $\phi = \bigwedge_i\phi_i \wedge \phi_s$ remains
$\forall^2\exists$.  The fragment of FOL that admits formulas with
prenex signatures of form $\forall^2\exists^*$ is called the
Godel-Kalmar-Schutte ({\sf GKS}) fragment~\cite{...}, which is known to be
decidable. The language of encoding, however, is a combination of {\sf
GKS} with (a). $\Prop^0$, the theory from which quantifier-free
propositions ($\varphi$) that encode object language expressions are
drawn, and (b).  $\Prop^1$, the theory from which invariants ($I$) are
drawn. Thus the encoding is decidable if the combination of ${\sf GKS}
+ \Prop^0 + \Prop^1$ is decidable.

To formally state the above property, let $\SL[n,\Prop^0,\Prop^1]$
denote the core set expression language that admits $n$ nested {\sf bind}
expressions, and allows program (Boolean) expressions ($e$), and
invariants ($\I$) to be drawn from theories $\Prop^0$ and $\Prop^1$,
respectively. Then:
\begin{theorem}
  $\SL[0,\Prop^0,\Prop^1]$ is decidable if ${\sf GKS}+\Prop^0+\Prop^1$
  is decidable.
\end{theorem}
The $n=0$ case denotes the lack of nested {\sf bind}
expressions. Since nested \C{foreach} statements are the only program
constructs in $\txnimp$ that lead to nested bind expressions, this
instantiation denotes the encoding language for $\txnimp$ programs
without nested \C{foreach} loops. Observe that the \C{new\_order}
transaction (Fig.~\ref{fig:new_order_code}) satisfies this constraint.
Indeed, this restricted language is nonetheless expressive enough to
cover all of the benchmarks we have considered.

A useful instantiation of $\SL$ is $\SL[{\sf BV},{\sf GKS}+{\sf BV}]$,
where ${\sf BV}$ is the theory of bit-vector arithmetic, which is
generally used to encode the finite-bit arithmetic of real programs.
Bit-vector arithmetic has a finite quantifier-free
axiomatization, 
