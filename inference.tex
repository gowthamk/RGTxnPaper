...
of $\bind$ adds one to $|\nubar|$, if $s$ is a bind expression nested
inside a top-level bind expression (like $\F_U(\stl,\stg)$ from
the above example), then the prenex signature of $\phi_s$ is
$\forall^2\exists$.  Furthermore, if the subexpressions of $s$ are
neither {bind} nor $\existsl$ expressions, then none of the
$\phi_i$'s are quantified, and the prenex signature of $\phi =
\bigwedge_i\phi_i \wedge \phi_s$ remains $\forall^2\exists$. Similar
...
a non-nested bind expression. $s'$ is however nested inside the
...
The fragment of FOL that admits formulas with prenex signatures of
...
points to the existence of subsets in $\SL[\Prop^0,\Prop^1]$ that are
decidable:
\begin{theorem}
  ...
  decidable.
\end{theorem}
An example of such an $\SL'$ is the subset described above: $\SL$ with
...
\begin{theorem}
  $\SL^1[{\sf GKS},{\sf GKS}+{\sf BV}]$ is decidable.
\end{theorem}
The $\SL^1[{\sf BV},{\sf GKS}+{\sf BV}]$ instantiation requires $I$ to
be drawn from {\sf GKS}+{\sf BV}, which is expressive enough to
...
already simple first-order formulas that can be encoded in {\sf GKS}.
...
Thus, with $\SL^1[{\sf GKS},{\sf GKS}+{\sf BV}]$ as the language of
