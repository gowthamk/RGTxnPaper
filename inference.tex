...
expression. Since $\existsl$ is a result of a particular choice of the
weakening semantics (Sec.~\ref{sec:inference}), let us ignore it to
focus on the core set expression language. This leaves $\bind$ as the
only case where $\nubar$ is extended.  Since an occurance of $\bind$
adds one to $|\nubar|$, if $s$ is a subexpression of a top-level
$\bind$ expression (like $\F_U(\stl,\stg)$ from the above example),
then the prenex signature of $\phi_s$ is $\forall^2\exists$.
Furthermore, if the subexpressions of $s$ are not $\bind$ expressions,
...
prenex signatures of form $\forall^2\exists^*$ is called
Godel-Kalmar-Schutte ({\sf GKS}) fragment, which is known to be
...
denote the core set expression language that admits $n$ nested $\bind$
expressions, and allows program's (boolean) expressions ($e$), and
...
\begin{theorem}
  ...
  is decidable.
\end{theorem}
The $n=0$ case denotes the lack of nested bind expressions. Since
nested \C{foreach} statements are the only program constructs in
$\txnimp$ that lead to nested bind expressions, the $n=0$ case
essentially denotes the encoding language for $\txnimp$ programs
...
Indeed, the restricted language without nested \C{foreach} loops is
nonetheless expressive enough to cover all TPC-C transactions.
