...
\rulelabel{RG-Conseq} rule for transaction-bound commands (cf.  the
...
\end{figure}
Here, an \C{add\_interest} transaction adds a positive interest to the
...
starts by issuing a \C{select1} query, whose effect implicitly asserts
...
that \C{add\_interest} preserves the positive balance invariant.
