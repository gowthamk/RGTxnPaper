...
\underE{\C{SI(Wd1)}}$. We assume an {\sc sc} store, such as a
...
that the language of our specifications and (most of) the assertions
on executions is a decidable subset of the first-order logic (in
particular, our specifications and rely-guarantee rules are carefully
crafted to avoid transitive closures and quantification over infinite
domains). Consequently, the proof obligations generated by the
application of our rules can often be discharged via an SMT solver. We
nonetheless give full details in the following.
