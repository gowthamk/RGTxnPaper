...
consistency commitment\footnote{The term \emph{recency
    commitment}~\cite{bailisvldb} is often used in practice
...
from being applied to stores that provide guarantees weaker than
causal consistency (e.g., \cite{bayou,pldi15}).
...
change,~\cite{gotsmanpopl16} distinguishes between an \emph{effect
  generation} event at one replica and an \emph{effect merge} event at
...
all such machinery into a stability condition (\S\ref{sec:rely-guarantee}).
