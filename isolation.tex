...
\C{UNIQUE} and \C{AUTO\_INCREMENT} keywords. Our development crucially
...
are generated locally by the \rulelabel{E-Insert} rule. Global
...
\textbf{Write-Write Conflicts}. Databases often employ a combination of
...
We now define various isolation guarantees that are either found in
...
RR allows \emph{phantom reads} phenomenon, where a \C{SELECT} query
...
particular, the records returned by \C{SELECT} should be added to the
