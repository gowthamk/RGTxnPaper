...
to the size of the \C{item\_req} set, which in turn is equal to the
...
not be any write-write conflicts (i.e,, both transactions modifying
the same record) among concurrent transactions when they
commit. Write-write conflicts can be eliminated in various ways, e.g.,
through conflict detection followed by a rollback, or through
