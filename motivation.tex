...
\subcaptionbox {
  ...
  \includegraphics[scale=0.5]{Figures/hb-eg-3}
}
\subcaptionbox {
  \label{fig:hb-eg-4}
}{
  \includegraphics[scale=0.5]{Figures/hb-eg-4}
}


\caption{\small For the example in Fig.~\ref{fig:motiv-eg-1},
$\psi_{SI}$ specification allows $\hbZ$ arrows (hence the execution)
in Fig.~\ref{fig:hb-eg-1}. $\psi_{RC}$ specification allows $\hbZ$
arrows in
Figs.~\ref{fig:hb-eg-1},~\ref{fig:hb-eg-2},~\ref{fig:hb-eg-3}, and
~\ref{fig:hb-eg-4}. Each arrow represents an $\hbZ$ relationship
between all operations of \C{Wd2} and an operation in \C{Wd1}. Letters
R, W and C stand for read, write and commit, respectively.}
