...
\subcaptionbox {
  ...
  \includegraphics[scale=0.6]{Figures/hb-eg-1}
}
\subcaptionbox {
  \label{fig:hb-eg-2}
} [
  ...
  \includegraphics[scale=0.6]{Figures/hb-eg-2}
}
\subcaptionbox {
  \label{fig:hb-eg-3}
} [
  ...
  \includegraphics[scale=0.6]{Figures/hb-eg-3}
}
\subcaptionbox {
  \label{fig:hb-eg-4}
}{
  \includegraphics[scale=0.6]{Figures/hb-eg-4}
}


\caption{\small $\hbZ$ arrows allowed by $\psi_{SI}$
 and $\psi_{RC}$ specifications
% (Figs.~\ref{fig:hb-eg-1},~\ref{fig:hb-eg-2},~\ref{fig:hb-eg-3},~\ref{fig:hb-eg-4})
for the example in Fig.~\ref{fig:motiv-eg-1}. Each
arrow represents an $\hbZ$ relationship between all operations of
\C{Wd2} and an operation in \C{Wd1}. Letters R, W and C stand for
...
operation $\eta_1$ from \C{Wd1} happen-before $\eta_2$ of \C{Wd2}
without the commit of \C{Wd1} also happening before $\eta_2$.
$\psi_{SI}$ however disallows $\hbZ$ arrows (hence executions) in
Figs.~\ref{fig:hb-eg-2},~\ref{fig:hb-eg-3},~\ref{fig:hb-eg-4} because
such arrows establish $\hbZ$ edges
between (operations in) \C{Wd2} and only a subset of the operations in
...
arrows in Figs.~\ref{fig:hb-eg-1} and~\ref{fig:hb-eg-2} denote
$\hbZ$ relationships that do not affect the value of \C{B} in a way
that causes the program to violate its post-condition. In contrast,
arrows in Figs.~\ref{hb-eg-3} and~\ref{hb-eg-4} denote $\hbZ$
...
previously committed transactions. {\sc sc} semantics can be built
...
useful, it has to be parametric over different consistency semantics,
supporting stores that are weaker than {\sc sc} (e.g., causally or
eventually consistent), and should be able to reconcile conflicts
between consistency and isolation constraints.  We demonstrate how our
reasoning framework makes this possible in the following sections.
