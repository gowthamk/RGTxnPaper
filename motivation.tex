...
behavior of the \C{new\_order} transaction when executed under
...
Thus, with two concurrent instances of the \C{new\_order} transaction
(call them $T_1$ and $T_2$), both concurrently placing new orders for
different customers under the same district (\C{d\_id}), RC isolation
...
operations, and the \C{foreach} combinator becoms set monad's bind
