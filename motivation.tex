...
style reasoning~\cite{rgjones}. Rely-guarantee is a compositional
proof technique that allows us to reason about one thread at a time by
abstracting away interference induced by other threads (collectively
...
constitutes an interference and what does not is a non-trivial
problem. For example, inside a serializable transaction there is no
interference, wherase inside a transaction executing under \iso{Read
...
admit some interferences while prohibiting others.
...
For the rely-guarantee approach to be useful, it has to thus constrain the
...
constraining the definition of the \emph{happens-before} ($\hbZ$) relation
between transactions, or their respective constituents.  To do so, we
axiomatize  the $\hbZ$ relation to capture the interference characteristics of
different isolation levels as their specifications ($\psi$). For example,
a \iso{Read Committed} specification allows an operation $\eta_1$ of
...
write $T_1 \hboar \eta_2$ to denote this). The {\sc rc} specification does not require
$T_1$ to execute and commit before all operations of $T_2$, thus allowing it to
interfere in $T_2$. The  \iso{Snapshot
Isolation} specification requires two transactions that write to the same
variable to be related by happens-before.  This effectively prohibits
interference due to actions in $T_1$ being interleaved in $T_2$ (or, vise versa).
Formally\footnote{Note that specifications are simplified for illustration. The
actual specifications described in (\S~\ref{sec:opsem}) are more nuanced.  }:
\begin{smathpar}
\begin{array}{lcl}
...
after the write to \C{C} in {\bf Wd1}. The expected invariant ($\phi$) at
...
shown by demonstrating that an interference from {\bf Wd2} starting from a
