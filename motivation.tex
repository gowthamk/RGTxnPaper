...
a \emph{Read Committed} ({\sc rc}) isolation level, which is weaker than
...
that executes it. In the first execution, transaction \C{Wd1} reads
the current balance (\C{k}) and writes the new balance (\C{k-a1}), but
...
in an incorrect balance of \C{k-a1}. The second execution describes a
similar scenario with \C{Wd1} and \C{Wd2} exchanging their roles.
...
interfere in $T_2$. The \iso{Snapshot Isolation} specification
...
from \C{Wd2} to \C{Wd1} allowed by $\psi_{RC}$ and $\psi_{SI}$. All
arrows are legal under $\psi_{RC}$ because in no case does an operation
$\eta_1$ from \C{Wd1} happen-before $\eta_2$ of \C{Wd2} without the commit
of \C{Wd1} also happening before $\eta_2$. $\psi_{SI}$ however disallows
all dashed arrows, because dashed arrows establish $\hbZ$ between actions in \C{Wd2}
and only a subset of the operations in \C{Wd1}. Lighter (blue) arrows with
...
to \C{C} in \C{Wd1}. The expected invariant ($\phi$) at that program
point is shown below ($\committed$ stands for ``committed''):\vspace*{-8pt}

\begin{smathpar}
\begin{array}{c}
 \neg\committed(\C{Wd1}) \conj \C{Wd1} \wrstoar \C{C}  \conj  \\
    (\neg\committed(\C{Wd2}) \Rightarrow \C{C = k-a1}) \conj (\committed(\C{Wd2})
                \Rightarrow \C{C = k-a1-a2})
\end{array}
\end{smathpar}
\vspace*{-8pt}

\noindent $\phi$ asserts that \C{Wd1} is not yet committed, and that it wrote to
\C{C}, and the value of \C{C} is either \C{k-a1-a2} or \C{k-a1}
depending on whether or not \C{Wd2} is committed. If $\phi$ remains
invariant until \C{Wd1} commits, then the post-condition (\C{C =
...
$\C{k-a1}$ to \C{C} and committing \C{Wd2}, thus leading to
$\committed(\C{Wd2}) \Rightarrow \C{C=k-a2}$. Fortunately,
\iso{Snapshot Isolation} prevents this interference, and this can be
shown by demonstrating that an interference from \C{Wd2} starting from an
...
from \C{Wd1} has happened before at least one operation of \C{Wd2}, and
the latter does not hold because \C{Wd1} has not yet committed, and \C{Wd2}
has already begun). Since \C{Wd2} also writes to \C{C}, this violates
