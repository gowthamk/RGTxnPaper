...
\iso{Read Uncommitted}, the weakest isolation level, all interferences
...
write $T_1 \hboar \eta_2$ to denote this). The {\sc rc} specification
does not require $T_1$ to execute and commit before all operations of
$T_2$, thus allowing it to interfere in $T_2$. The \iso{Snapshot
  Isolation} specification requires two transactions that write to the
...
$T_2$ (or, vise versa).  Formally\footnote{Note that specifications
  are simplified for illustration. The actual specifications described
  in (\S~\ref{sec:opsem}) are more nuanced.  }:
\begin{smathpar}
\begin{array}{lcl}
