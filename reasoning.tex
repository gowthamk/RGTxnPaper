...
\end{array}
\end{smathpar}
In words, if there exists $\stg_1$ which was the global database state
during the last step by a transaction and if the transaction can take
its next step at the current global database state $\stg$, then even
after the interference, the transaction should still be able to take
its next step. If there is no such state $\stg_1$, then the
transaction will be stuck, and the interference can be safely
discarded. Often, this significantly simplifies the form of $\R_l$
irrespective of $R$. For example, when a transaction is executed
against a snapshot of the database (i.e. $\mathbb{I}_{ss}$), $\R_l$
will be the identity function, since if $\stg \neq \stg'$, there does
not exist any $\stg_1$ which satisfies the constraint for
$\R_l(\stl,\stg,\stg')$.
