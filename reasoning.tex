...
transaction-local judgment to verify the transaction body ($c$).
...
\end{array}
\end{smathpar}
In words, if there exists $\stg_1$ which was the global database state during the last step by a transaction and if the transaction can take its next step at the current global database state $\stg$, then even after the interference, the transaction should still be able to take its next step. If there is no such state $\stg_1$, then a transaction will never be able to take its next step after the interference, and hence such interference can be safely discarded. 
...
because the original record bears the id of a transaction that has
already committed. Thus, $\I_{ss}$ remains invalid.
...
Salient rules of transaction-local RG judgment are shown in
...
the iteration, hence $Q_c \conj z\in y$ is valid at the end of the
iteration, from which $\psi$ must follow. When the loop has finished
execution, $y$, the set of iterated items, is the entire set $x$. Thus
$\psi \conj y=x$ must imply the post-condition $Q$, which also needs
to be stable. The rules for conditionals, sequencing etc., are
...
\R \vdash \hoare{P}{c}{Q} & \defeq & \forall \stl,\stl',\stg,\stg'.~
  P(\stl,\stg) \conj (c,\stl,\stg) \rstepssto{n} (\cskip, \stl',\stg')
  \Rightarrow Q(\stl',\stg')\\
...
\begin{smathpar}
\begin{array}{lcl}
\texttt{step-guaranteed}(R,G,c,\stg) &\defeq& \forall n,\stg',c'',\stg''.
