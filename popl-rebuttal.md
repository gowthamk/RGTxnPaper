...
- Limited set of operations in the DSL:  While it is true our language
  ...
  TPC-E. We also note that MySQL's [developer
...
- Isolation levels weaker than RC: please refer to our comments on RU
  isolation above.
...
- Not admitting valid snapshot isolation (SI) executions:  We believe
  ...
  non-conflicting concurrent transactions to commit in parallel, hence
...
- Decidability: The RG-TXN (Sec. 5.2, p. 20) rule does not lead to
  ...
  function symbols that occur free in the encoded formula. For
  instance consider the ${\sf F} \Rightarrow G$ premise of RG-TXN. The
  quantified set variables $\delta$ and $\Delta$ are instantiated with
  fresh function variables $f_{\delta}$ and $f_{\Delta}$,
  respectively, resulting in a quantifier-free implication with free
  function symbols. The antecedent of this implication (${\sf F}$) is
  asserted as such, but its consequent ($G$) is negated before
  checking the satisfiability of the entire formula. This is a
  standard approach often employed to discharge the verification
  conditions of form $\forall a. \phi_1(a) \Rightarrow \phi_2(a)$.
  Note that in this process: (i). sets or functions are never
  quantified (thus no second-order quantification), and (ii). ${\sf
  F}$ is never negated (thus no inversion of quantifiers in ${\sf
  F}$). Other premises are similarly encoded.
